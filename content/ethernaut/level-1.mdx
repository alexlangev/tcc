...
an improper implementation of the `onlyOwner` logic can expose critical functions.
...
1. `msg.value` > 0 
2.  `contributions[msg.sender] > 0` is also simple. All we need is to increase the contribution of our EOA to a positive_
amount.
...
We can now send 1 Wei to the contract address. Doing this will make your EOA the `owner`. I used the Remix interface to do this 
but you can also do it directly in the console.
